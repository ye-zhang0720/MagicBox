...
void create_ui(void *phy_fb, int screen_width, int screen_height, int color_bytes, struct EXTERNAL_GFX_OP *gfx_op) // ui的初始化函数
{
    ...
    int hour = sys_rtc.getHour(false);
    int minute = sys_rtc.getMinute();
    int second = sys_rtc.getSecond();
    float sec_angle = 0.1047197551 * ((second + 45) % 60);
    float minute_angle = 0.1047197551 * ((minute + 45) % 60);
    float hour_angle = 0.52359877 * ((hour + 9) % 12) + minute * 0.00872665;
    /***********这里是自定义需要的初始化******/
    time_clock.set_hands(CLOCK_RADIUS, HOUR_HAND_LEN, hour_angle, HOUR_HAND_RGB, MINUTE_HAND_LEN, minute_angle, MINUTE_HAND_RGB, SECOND_HAND_LEN, sec_angle, SECOND_HAND_RGB);
    /*****************/
}
