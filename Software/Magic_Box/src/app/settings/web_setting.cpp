...
#define SYS_SETTING "<form method=\"GET\" action=\"saveSysConf\">"                                                                                                                                                                                                      \
                    ...
                    "<label class=\"input\"><span>自动调节屏幕亮度</span><input class=\"radio\" type=\"radio\" value=\"0\" name=\"auto_backLight\" %s>关闭<input class=\"radio\" type=\"radio\" value=\"1\" name=\"auto_backLight\" %s>开启</label>"        \
...
#define WEATHER_SETTING "<form method=\"GET\" action=\"saveWeatherConf\">"                                                                                          \
                        ...
                        "<label class=\"input\"><span>天气更新周期（小时）</span><input type=\"text\"name=\"weatherUpdataInterval\"value=\"%s\"></label>" \
                        "</label><input class=\"btn\" type=\"submit\" name=\"submit\" value=\"保存\"></form><label>天气服务注册地址: https://www.yiketianqi.com/</label>"
...
#define ROUTER_MONITOR_SETTING "<form method=\"GET\" action=\"saveRouterMonitorConf\">"                                                                  \
                               "<label class=\"input\"><span>路由器地址</span><input type=\"text\"name=\"router_host\"value=\"%s\"></label>"        \
                               "<label class=\"input\"><span>端口号</span><input type=\"text\"name=\"router_port\"value=\"%s\"></label>"              \
                               "<label class=\"input\"><span>内存容量(M)</span><input type=\"text\"name=\"router_mem_size\"value=\"%s\"></label>"      \
                               "<label class=\"input\"><span>CPU Chart ID</span><input type=\"text\"name=\"router_CPU\"value=\"%s\"></label>"            \
                               "<label class=\"input\"><span>内存 Chart ID</span><input type=\"text\"name=\"router_mem\"value=\"%s\"></label>"         \
                               "<label class=\"input\"><span>网络 Chart ID</span><input type=\"text\"name=\"router_network\"value=\"%s\"></label>"     \
                               "<label class=\"input\"><span>温度 Chart ID</span><input type=\"text\"name=\"router_temperature\"value=\"%s\"></label>" \
                               "</label><input class=\"btn\" type=\"submit\" name=\"submit\" value=\"保存\"></form>"
...
void init_page_header()
{
    ...
    webpage_header += F("<title>MagicBox Setting</title>"); // NOTE: 1em = 16px
    ...
    webpage_header += F("<h1>MagicBox_AIO ");
    ...
    webpage_header += F("<li><a href='/router_monitor_setting'>路由器监控</a></li>");
#endif
    webpage_header += F("</ul>");
}
...
{
    webpage_footer = F("<footer>&copy;西北偏北 2023</footer>");
...
void HomePage()
{
    ...
    webpage += F("<a href='https://github.com/ClimbSnail/MagicBox_AIO' target='_blank'><button>Github</button></a>");
...
void sys_setting()
{
    ...
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "AppCtrl", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SETTINGS_APP_NAME, "AppCtrl", APP_MESSAGE_GET_PARAM,
                            (void *)"ssid_0", ssid_0);
    app_controller->send_to(SETTINGS_APP_NAME, "AppCtrl", APP_MESSAGE_GET_PARAM,
...
void weather_setting()
{
    ...
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "天气时钟", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SETTINGS_APP_NAME, "天气时钟", APP_MESSAGE_GET_PARAM,
...
void weather_old_setting()
{
    ...
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "Weather Old", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SETTINGS_APP_NAME, "Weather Old", APP_MESSAGE_GET_PARAM,
...
void bili_setting()
{
    ...
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "Bili", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SETTINGS_APP_NAME, "Bili", APP_MESSAGE_GET_PARAM,
...
void stock_setting()
{
    ...
    char updataInterval[32];
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "股票", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SETTINGS_APP_NAME, "股票", APP_MESSAGE_GET_PARAM,
...
void picture_setting()
{
    ...
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "照片", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SETTINGS_APP_NAME, "照片", APP_MESSAGE_GET_PARAM,
...
void media_setting()
{
    ...
    char powerFlag[32];
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "媒体", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SETTINGS_APP_NAME, "媒体", APP_MESSAGE_GET_PARAM,
...
void screen_setting()
{
    ...
    char powerFlag[32];
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "屏幕共享", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SETTINGS_APP_NAME, "屏幕共享", APP_MESSAGE_GET_PARAM,
                            (void *)"powerFlag", powerFlag);
...
void heartbeat_setting()
{
    ...
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "Heartbeat", APP_MESSAGE_READ_CFG,
                            NULL, NULL);

    app_controller->send_to(SETTINGS_APP_NAME, "Heartbeat", APP_MESSAGE_GET_PARAM,
                            (void *)"role", role);
    app_controller->send_to(SETTINGS_APP_NAME, "Heartbeat", APP_MESSAGE_GET_PARAM,
...
void anniversary_setting()
{
    ...
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "Anniversary", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SETTINGS_APP_NAME, "Anniversary", APP_MESSAGE_GET_PARAM,
...
void pc_resource_setting()
{
    ...
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "电脑仪表盘", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SETTINGS_APP_NAME, "电脑仪表盘", APP_MESSAGE_GET_PARAM,
...
void router_monitor_setting()
{
    ...
    char mem_size[8];
    // 读取数据
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控", APP_MESSAGE_READ_CFG,
                            NULL, NULL);
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控", APP_MESSAGE_GET_PARAM,
                            (void *)"host", host);
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控", APP_MESSAGE_GET_PARAM,
                            (void *)"port", port);
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控", APP_MESSAGE_GET_PARAM,
    ...
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控", APP_MESSAGE_GET_PARAM,
                            (void *)"mem_size", mem_size);

    sprintf(buf, ROUTER_MONITOR_SETTING, host, port, mem_size, cpu, mem, network, temperature);
    webpage = buf;
    Send_HTML(webpage);
}
...
void saveSysConf(void)
{
    ...
    app_controller->send_to(SETTINGS_APP_NAME, "AppCtrl",
    ...
    app_controller->send_to(SETTINGS_APP_NAME, "AppCtrl",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"auto_backLight",
                            (void *)server.arg("auto_backLight").c_str());
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "AppCtrl", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
...
void saveWeatherConf(void)
{
    ...
    app_controller->send_to(SETTINGS_APP_NAME, "天气时钟",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"tianqi_appid",
    ...
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "天气时钟", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}
...
void saveWeatherOldConf(void)
{
    ...
    app_controller->send_to(SETTINGS_APP_NAME, "Weather Old",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"cityname",
    ...
    app_controller->send_to(SETTINGS_APP_NAME, "Weather Old",
                            ...
                            (void *)server.arg("timeUpdataInterval").c_str());
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "Weather Old", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}
...
{
    Send_HTML(F("<h1>设置成功! 退出APP或者继续其他设置.</h1>"));
    app_controller->send_to(SETTINGS_APP_NAME, "Bili",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"bili_uid",
    ...
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "Bili", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}
...
{
    Send_HTML(F("<h1>设置成功! 退出APP或者继续其他设置.</h1>"));
    app_controller->send_to(SETTINGS_APP_NAME, "股票",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"stock_id",
    ...
    app_controller->send_to(SETTINGS_APP_NAME, "股票",
                            ...
                            (void *)server.arg("updataInterval").c_str());
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "股票", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}
...
{
    Send_HTML(F("<h1>设置成功! 退出APP或者继续其他设置.</h1>"));
    app_controller->send_to(SETTINGS_APP_NAME, "照片",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"switchInterval",
                            (void *)server.arg("switchInterval").c_str());
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "照片", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}
...
{
    Send_HTML(F("<h1>设置成功! 退出APP或者继续其他设置.</h1>"));
    app_controller->send_to(SETTINGS_APP_NAME, "媒体",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"switchFlag",
    ...
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "媒体", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}
...
{
    Send_HTML(F("<h1>设置成功! 退出APP或者继续其他设置.</h1>"));
    app_controller->send_to(SETTINGS_APP_NAME, "屏幕共享",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"powerFlag",
                            (void *)server.arg("powerFlag").c_str());
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "屏幕共享", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}
...
{
    Send_HTML(F("<h1>设置成功! 退出APP或者继续其他设置.</h1>"));
    app_controller->send_to(SETTINGS_APP_NAME, "Heartbeat",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"role",
    ...
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "Heartbeat", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}
...
{
    Send_HTML(F("<h1>设置成功! 退出APP或者继续其他设置.</h1>"));
    app_controller->send_to(SETTINGS_APP_NAME, "Anniversary",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"event_name0",
    ...
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "Anniversary", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}
...
{
    Send_HTML(F("<h1>设置成功! 退出APP或者继续其他设置.</h1>"));
    app_controller->send_to(SETTINGS_APP_NAME, "电脑仪表盘",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"pc_ipaddr",
    ...
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "电脑仪表盘", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}

void saveRouterMonitorConf(void)
{
    Send_HTML(F("<h1>设置成功! 退出APP或者继续其他设置.</h1>"));
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"host",
                            (void *)server.arg("router_host").c_str());
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"port",
                            (void *)server.arg("router_port").c_str());
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"cpu",
                            (void *)server.arg("router_CPU").c_str());
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"mem",
                            (void *)server.arg("router_mem").c_str());
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"network",
                            (void *)server.arg("router_network").c_str());
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"temperature",
                            (void *)server.arg("router_temperature").c_str());
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"mem_size",
                            (void *)server.arg("router_mem_size").c_str());
    // 持久化数据
    app_controller->send_to(SETTINGS_APP_NAME, "路由器监控", APP_MESSAGE_WRITE_CFG,
                            NULL, NULL);
}
