...
void AppController::read_config(SysUtilConfig *cfg)
{
    ...
    if (size == 0)
    {
        ...
        cfg->auto_backLight = 1;       // 是否自动调节背光 0关闭 1打开
        cfg->mpu_order = 10;           // 操作方向
    ...
    else
    {
        ...
        cfg->auto_backLight = atol(param[12]);
    }
}

void AppController::write_config(SysUtilConfig *cfg)
{
    sys_cfg.ssid_0 = cfg->ssid_0;
    sys_cfg.password_0 = cfg->password_0;
    char tmp[25];
    // 将配置数据保存在文件中（持久化）
    String w_data;
    ...
    Serial.println("write config to sd");
    // 立即生效相关配置
    setBackLight(cfg->backLight / 100.0);
...
void AppController::write_config(SysMpuConfig *cfg)
{
    ...
    Serial.println("MPU6050 write config to sd");
}
...
void AppController::read_config(RgbConfig *cfg)
{
    ...
    if (size == 0)
    {
        ...
        cfg->step_1 = 1;
        cfg->step_2 = 1;
        cfg->min_brightness = 0.03;
        cfg->max_brightness = 0.15;
                                ...
                                const char *key, char *value)
{
    ...
    case APP_MESSAGE_GET_PARAM:
    {
        ...
        else if (!strcmp(key, "auto_backLight"))
        {
            snprintf(value, 32, "%u", sys_cfg.auto_backLight);
        }
    }
    break;
    case APP_MESSAGE_SET_PARAM:
    {
        ...
        {
            sys_cfg.auto_backLight = atol(value);
        }
    }
    break;
