...
void app_control_gui_init(void)
{
    ...
    lv_style_set_text_font(&app_name_style, &weiruanyahei_27);
...
static void label_event_cb(lv_event_t *e)
{
    ...
    if (strstr(fmt, "%s") != NULL)
    {
        ...
        lv_obj_align_to(label, pre_app_image, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);
    }
    else
    {
        /* 数字填入 */
...
void change_loading_msg(char* str)
{
    lv_msg_send(APP_NAME, str);
}
...
void display_app_scr_init(const void *src_img_path, const char *app_name)
{
    ...
    lv_label_set_long_mode(pre_app_name, LV_LABEL_LONG_WRAP);
    // lv_obj_set_size(pre_app_name, 240, 30);
    lv_msg_subsribe_obj(APP_NAME, pre_app_name, "%s");
    lv_obj_add_event_cb(pre_app_name, label_event_cb, LV_EVENT_MSG_RECEIVED, NULL);
