...
void AppController::init(void)
{
    ...
    appList[0]->app_name = "正在校准陀螺仪, 请勿移动";
...
void AppController::event_handle()
{
    if (isRunEventDeal)
    {
...
bool AppController::wifi_event(APP_MESSAGE_TYPE type)
{
    ...
    case APP_MESSAGE_WIFI_CONN:
    {
        ...
        if ((false == m_wifi_status) || WL_CONNECTED != WiFi.status())
        {
            if(sys_cfg.ssid_0.isEmpty())
            {
                Serial.print(F("[EVENT]\tPlease set WI-FI SSID and Password\n"));
                return false;
            }
