...
const char *serverIndex =
    ...
    "                         document.getElementById('software').innerHTML = '软件版本： '+data.software;"
    "                document.getElementById('hardware').innerHTML = '硬件版本： '+data.hardware;"
    "                document.getElementById('flash').innerHTML = 'Flash容量： '+data.flash + 'M';"
    "               } "
    "               });  "
...
static int ota_init(AppController *sys)
{
    ...
    AIO_LVGL_OPERATE_LOCK(show_ota_result_messageBox("等待Wi-Fi连接"));
    return 0;
}
                        ...
                        const ImuAction *action)
{
    ...
    if (WL_CONNECTED == WiFi.status() &&run_data->web_start != 1)
    {
        AIO_LVGL_OPERATE_LOCK(close_ota_result_messageBox());
        display_ota(
            "访问下面ip地址, 设置APP",
