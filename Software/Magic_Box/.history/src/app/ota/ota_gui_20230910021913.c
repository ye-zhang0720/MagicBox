...
void display_ota_init(void)
{
    ...
    lv_obj_t *barLabel = lv_label_create(scr_1);
    lv_obj_add_style(barLabel, &chFont_style, LV_STATE_DEFAULT);
    lv_label_set_text_fmt(barLabel, "%2d°C", 0);
    ...
    lv_msg_subsribe_obj(Bar, tempLabel, "%d°C");
    lv_obj_add_event_cb(tempLabel, label_event_cb, LV_EVENT_MSG_RECEIVED, NULL);
    lv_msg_subsribe_obj(Bar, tempBar, NULL);
