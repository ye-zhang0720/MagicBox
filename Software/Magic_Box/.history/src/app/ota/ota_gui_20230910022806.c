...
void display_ota_init(void)
{
    ...
    lv_label_set_text_fmt(barLabel, "进度：%2f%%", 0);
    lv_obj_align(barLabel, LV_ALIGN_CENTER, 0, 0);
    ...
    lv_msg_subsribe_obj(Bar, barLabel, "进度：%2d%%");
...
void update_progress(uint8_t a)
{
    lv_msg_send(Bar, &humi);
}
