...
struct ServerAppRunData
{
    ...
    TaskHandle_t xHandle_update_task_update;
    // HTTPUpload& upload;
};
        ...
        []()
        {
            ...
            else if (upload.status == UPLOAD_FILE_WRITE)
            {
                ...
                if (Update.write(upload.buf, upload.currentSize) != upload.currentSize)
                {
                 ...
                 Serial.printf("Update  %u   %u   %d\n", upload.totalSize, Update.size(), (uint8_t)(upload.totalSize/ Update.size()));
                // if (upload.totalSize != 0)
                // {
                    AIO_LVGL_OPERATE_LOCK(update_progress((uint8_t)(upload.totalSize/ Update.size())););
                // }
            }
...
static int ota_init(AppController *sys)
{
    ...
    run_data->xHandle_update_task_update = xTaskCreate(
        task_update,                          /*任务函数*/
                        ...
                        const ImuAction *action)
{
    ...
    //                     if (upload.totalSize != 0)
    //             {
    //                 AIO_LVGL_OPERATE_LOCK(update_progress(upload.currentSize / upload.totalSize););
    //             }
    // }
}
