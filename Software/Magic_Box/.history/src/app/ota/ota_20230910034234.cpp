...
static void start_ota_web_config()
{
    ...
    ESP.restart(); },
        ...
        {
            // show_progress();
            run_data->upload = server.upload();
            if (run_data->upload.status == UPLOAD_FILE_START)
            {
                Serial.printf("Update: %s\n", run_data->upload.filename.c_str());
                if (!Update.begin(UPDATE_SIZE_UNKNOWN))
                { // start with max available size
                    Update.printError(Serial);
                }
            }
            else if (run_data->upload.status == UPLOAD_FILE_WRITE)
            {
                /* flashing firmware to ESP*/
                if (Update.write(run_data->upload.buf, run_data->upload.currentSize) != run_data->upload.currentSize)
                {
                    Update.printError(Serial);
                }
            }
            else if (run_data->upload.status == UPLOAD_FILE_END)
            {
                if (Update.end(true))
                { // true to set the size to the current progress
                    Serial.printf("Update Success: %u\nRebooting...\n", run_data->upload.totalSize);
                    AIO_LVGL_OPERATE_LOCK(show_result_messageBox("更新成功, 正在重启"));
                }
                else
                {
                    Update.printError(Serial);
                    AIO_LVGL_OPERATE_LOCK(show_result_messageBox("更新失败!!"));
                        ...
                        const ImuAction *action)
{
    ...
    // if (run_data->ota_server_start == 1)
    // {
    //     update_progress(run_data->upload.currentSize / run_data->upload.totalSize);
    // }

    if (run_data->upload.totalSize != 0)
    {
        AIO_LVGL_OPERATE_LOCK(update_progress(run_data->upload.currentSize / run_data->upload.totalSize););
    }
}
